    ...
    private suspend fun setData(urlProfile: String) = withContext(Dispatchers.IO) {
        ...
        client.newCall(request).execute().use { response ->
            ...
            val document = Jsoup.parse(response.body?.string()).select("div[class=prof-page-wrapper]")
                ...
                val urlImgVet = document.select("div[class=prof-page__header-wrapper]").select("div[class=prof-page__header]").select("div[class=prof-page__header-photo z-placeholder z-placeholder--60 z-skeleton]").select("img").attr("data-original")

                val clinicName= document.select("div[class=service-page bg-gray js-phone-holder pd-m clearfix]").select("div[class=ss-container flexbox]").select("div[class=oh]").select("div[class=service-box-white service-block oh]").select("ul[class=list-reset service-items-medium service-items-medium-hovered btop js-results-container]").select("li[class=minicard-item js-results-item  ]").select("div[class=minicard-item__container]").select("div[class=minicard-item__info]").select("div[class=minicard-item__info]").select("h2").text()
                val clinicGrade= document.select("div[class=z-text--bold]").text()
                val clinicAdress = document.select("div[class=service-page bg-gray js-phone-holder pd-m clearfix]").select("div[class=ss-container flexbox]").select("div[class=oh]").select("div[class=service-box-white service-block oh]").select("ul[class=list-reset service-items-medium service-items-medium-hovered btop js-results-container]").select("li[class=minicard-item js-results-item  ]").select("div[class=minicard-item__container]").select("div[class=minicard-item__info]").select("div[class=minicard-item__info]").select("address[class=minicard-item__address]").text()
                val clinicImg = document.select("div[class=service-page bg-gray js-phone-holder pd-m clearfix]").select("div[class=ss-container flexbox]").select("div[class=oh]").select("div[class=service-box-white service-block oh]").select("ul[class=list-reset service-items-medium service-items-medium-hovered btop js-results-container]").select("li[class=minicard-item js-results-item  ]").select("div[class=minicard-item__container]").select("div[class=minicard-item__photo]").select("a[class=photo-wrapper js-item-url js-minicard-photo]").select("div[class=slider-block js-slider-block]").attr("data-photos")
